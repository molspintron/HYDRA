...
//Initialize and returns energy grid
...
void print_orbitals(Eigen::MatrixXd mat, Eigen::VectorXcd eig, int norbs, double homo, int output, int xbf1, int nbas) 
{
    ...
    for (int i = 0; i < norbs; ++i) {
        ...
        molden << "Ene= " << real(eig(i) - homo) << std::endl;
...
int main(int argc, char *argv[]) {
	...
	std::vector<float> Param = read_input(infile, 0);
	infile.close();
    ...
    if (coup_switch == 1) {
		...
		b_arr = read_intvector(infile, nbas, 0);
        infile.close();
    }
    ...
    Coef = read_denmat(infile, nbas, 0, "coef");
    ...
    Eig = read_denmat(infile, nbas, 0, "eigenvalues");
    ...
    if (diag_param == 1) {
        D = diag_corr_orbs(Ham_ortho, xbf1, ncorrorbs, 0.0, nbas, 0);
        T = X * D;
    } else {
        T = X;
    }
    ...
    if (gf_param == 2) {
        P_C = buildproj(Over, Coef, nbas, Eig, diag_param, xbf1, ncorrorbs, X, 0);
    }
    ...
    if (gf_param == 1 || gf_param == 2) {
        Eigen::MatrixXcd my_gf_proj_dia = Eigen::MatrixXcd::Zero(ncorrorbs, points);
        Eigen::MatrixXcd my_D_lehmann = Eigen::MatrixXcd::Zero(ncorrorbs, points);
...
#pragma omp for private(tmp_direct_GF, tmp_proj_GF, temp_direct_Lehmann) schedule(static, 1)
            for (int k = 0; k < points; k++) {
				...
				for (int i = 0; i < ncorrorbs; i++) {
                    if (gf_param == 2) {
                        my_gf_proj_dia(i, k) = tmp_proj_GF(i, i);
                    } else if (gf_param == 1) {
                        my_gf_proj_dia(i, k) = tmp_direct_GF(xbf1 - 1 + i, xbf1 - 1 + i);
                    }
				}
				if (gf_param == 2) {
                ...
                my_D_lehmann.block(0, k, ncorrorbs, 1) = temp_direct_Lehmann;
			}
        }
